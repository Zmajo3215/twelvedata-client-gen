...
[**GetEdgarFilingsArchive**](AnalysisAPI.md#GetEdgarFilingsArchive) | **Get** /edgar_filings/archive | EDGAR fillings
...
func main() {
	...
	ratingChange := "ratingChange_example" // string | Filter by rating change action (optional)
 ...
 **ratingChange** | **string** | Filter by rating change action | 
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.AnalysisAPI.GetEdgarFilingsArchive(context.Background()).Symbol(symbol).Figi(figi).Isin(isin).Cusip(cusip).Exchange(exchange).MicCode(micCode).Country(country).FormType(formType).FilledFrom(filledFrom).FilledTo(filledTo).Page(page).PageSize(pageSize).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AnalysisAPI.GetEdgarFilingsArchive``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetEdgarFilingsArchive`: GetEdgarFilingsArchive200Response
	fmt.Fprintf(os.Stdout, "Response from `AnalysisAPI.GetEdgarFilingsArchive`: %v\n", resp)
}
```
